"""Диагностика источников данных switcher (Zabbix, ClickHouse, SNMP)."""
...
from mcp_servers.switcher.analyzer import (
    analyze_switch,
...
def _zabbix_request(method: str, params: dict, auth: str | None = None) -> dict:
    ...
    response = requests.post(
        f"{config.ZABBIX_URL}/api_jsonrpc.php",
        json=payload,
...
def probe_zabbix(switch_ip: str) -> dict[str, Any]:
    ...
    if not token:
        result["error"] = "Ошибка авторизации в Zabbix"
        return result
...
async def collect_switch_probe(
    ...
    result: dict[str, Any] = {
        ...
        "config": {
            ...
            "clickhouse": f"{config.CLICKHOUSE_HOST}:{config.CLICKHOUSE_PORT}/{config.CLICKHOUSE_DB}",
            "snmp_community_set": bool(config.SNMP_COMMUNITY),
        },
    }
